...
set VCVARSALL=""
for /f "delims=" %%i in ('where /R "%PROGRAMFILES%" vcvarsall.bat') do set VCVARSALL="%%i"
if %VCVARSALL%=="" (for /f "delims=" %%i in ('where /R "%PROGRAMFILES(X86)%" vcvarsall.bat') do set VCVARSALL="%%i")
